#!/usr/bin/env python3
"""Montesarchio Track Manager - PyQt5 pipeline manager with 3D preview."""
...
DEFAULT_KN5 = os.path.join(ROOT_DIR, "mod", "pista_caudina", "pista_caudina.kn5")
...
BUILD_STEPS = [
    ("1 - Export KN5", [BLENDER, "--background",
                        os.path.join(ROOT_DIR, "pista_caudina.blend"), "--python",
...
class PreviewTab(QWidget):
    def __init__(self, parent=None):
        ...
        # KN5 is auto-exported on startup by TrackManager;
        # preview is then reloaded via BuildTab._on_all_done.
...
class BuildTab(QWidget):
    ...
    def _on_step_finished(self, exit_code, _exit_status):
        ...
        if exit_code == 0:
            self._append_log(f"  ✓ {label} completato", "#55cc55")
            ...
            self._append_log(f"  ✗ {label} fallito (exit code {exit_code})", "#ff5555")
            self._queue.clear()
    ...
    def _on_all_done(self):
        ...
        if os.path.isfile(src_kn5):
            ...
            self._append_log(f"KN5 copiato in mod/", "#55cc55")
...
class InstallWorker(QThread):
    ...
    def _install_track(self):
        ...
        mod_src = os.path.join(ROOT_DIR, "mod", "pista_caudina")
        kn5_file = os.path.join(mod_src, "pista_caudina.kn5")
        if not os.path.isdir(mod_src):
            self._log("Errore: cartella mod/pista_caudina non trovata. Esegui prima la build.", "#ff5555")
            return
        if not os.path.isfile(kn5_file):
            self._log("Errore: pista_caudina.kn5 non trovato nella cartella mod.", "#ff5555")
            return
        dest = os.path.join(self.ac_dir, "content", "tracks", "pista_caudina")
        if os.path.isdir(dest):
            shutil.rmtree(dest)
        shutil.copytree(mod_src, dest)
        self._log("[PISTA] Pista Caudina installata!", "#55cc55")
    ...
    def _clean_caches(self):
        ...
        if os.path.isdir(ac_cache):
            for subdir in ("ai_grids", "ai_payloads"):
                pattern = os.path.join(ac_cache, subdir, "pista_caudina__*")
...
class InstallTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.chk_track = QCheckBox("Pista Caudina")
...
PARAM_DEFS = {
    "geometry": {
        "label": "Geometria",
        "params": [
            ("road_width",     "Road Width (m)",      "double", 7.5,  3.0,  20.0, 0.5),
            ("kerb_width",     "Kerb Width (m)",      "double", 1.2,  0.2,  3.0,  0.1),
    ...
    "ai_line": {
        "label": "AI Line",
        "params": [
            ("default_speed",    "Default Speed (km/h)",    "double", 80.0, 20.0, 200.0, 5.0),
            ("min_corner_speed", "Min Corner Speed (km/h)", "double", 40.0, 10.0, 100.0, 5.0),
        ],
    },
    "surfaces": {
        "label": "Superfici",
...
INFO_FIELDS = [
    ("name",      "Name",      "line",  "Pista Caudina"),
    ("city",      "City",      "line",  "Montesarchio"),
    ("country",   "Country",   "line",  "Italy"),
    ("length",    "Length (m)", "line",  "860"),
    ("pitboxes",  "Pitboxes",  "line",  "10"),
...
class ParametersTab(QWidget):
    def __init__(self, parent=None):
        ...
        for key, label, wtype, default in INFO_FIELDS:
            if wtype == "combo":
                widget = QComboBox()
                widget.addItems(["clockwise", "anticlockwise"])
...
class TrackManager(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Montesarchio Track Manager")
    ...
    def _auto_export_kn5(self):
        blend = os.path.join(ROOT_DIR, "pista_caudina.blend")
