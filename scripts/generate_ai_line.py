...
Run with: blender --background pista_caudina.blend --python scripts/generate_ai_line.py
...
OUTPUT_PATH = os.path.join(ROOT_DIR, "mod", "pista_caudina", "ai", "fast_lane.ai")
...
DEFAULT_SPEED = _ai.get("default_speed", 80.0)
MIN_CORNER_SPEED = _ai.get("min_corner_speed", 40.0)
...
def extract_centerline():
    ...
    # We want clockwise (CW) driving direction; reverse if CCW
    if signed_area > 0:
...
def find_start_index(pts_blender):
    ...
    # pos.x = Blender_X, pos.y = Blender_Y
...
def write_ai_file(centerline_blender, output_path):
    ...
    ac_pts[:, 0] = centerline_blender[:, 0]    # AC X = Blender X
    ac_pts[:, 1] = centerline_blender[:, 2]    # AC Y = Blender Z (height)
    ac_pts[:, 2] = -centerline_blender[:, 1]   # AC Z = -Blender Y
