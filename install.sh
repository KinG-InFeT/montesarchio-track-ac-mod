#!/bin/bash
# Pista Caudina - Installazione/Aggiornamento mod per Assetto Corsa
...
echo "=== Pista Caudina - Installazione mod ==="
echo "AC trovato in: $AC_DIR"
echo ""

# --- 1. Installazione pista Pista Caudina ---
MOD_NAME="pista_caudina"
MOD_FULL="$SCRIPT_DIR/mod/$MOD_NAME"
DEST="$TRACKS_DIR/$MOD_NAME"

if [ ! -d "$MOD_FULL" ]; then
    echo "Errore: cartella mod pista non trovata in $MOD_FULL"
    echo "Esegui prima la pipeline di build (vedi README.md)"
    exit 1
fi

if [ ! -f "$MOD_FULL/$MOD_NAME.kn5" ]; then
    echo "Errore: $MOD_NAME.kn5 non trovato nella cartella mod."
    echo "Esegui prima: /snap/bin/blender --background pista_caudina.blend --python scripts/export_kn5.py"
    echo "Poi: cp pista_caudina.kn5 mod/$MOD_NAME/"
    exit 1
fi

echo "[PISTA] Installando Pista Caudina..."
if [ -d "$DEST" ]; then
    rm -rf "$DEST"
fi
cp -r "$MOD_FULL" "$DEST"
echo "[PISTA] Pista Caudina installata!"

# Clean AC engine cache for this track (AI grids, payloads, mesh metadata)
AC_CACHE="$AC_DIR/cache"
if [ -d "$AC_CACHE" ]; then
    cleaned=0
    for f in "$AC_CACHE"/ai_grids/pista_caudina__* "$AC_CACHE"/ai_payloads/pista_caudina__*; do
        if [ -f "$f" ]; then
            rm -f "$f"
            cleaned=$((cleaned + 1))
        fi
...
if [ -f "$CSP_DLL" ] && [ -d "$CSP_EXT" ]; then
    ...
    if [ -f "$CSP_ZIP" ]; then
        ...
        # CSP contiene dwrite.dll e cartella extension/
...
echo ""
echo "Pista:"
if [ -d "$TRACKS_DIR/pista_caudina" ]; then
    kn5=$(find "$TRACKS_DIR/pista_caudina" -name "*.kn5" -maxdepth 1 | head -1)
    if [ -n "$kn5" ]; then
        size=$(du -h "$kn5" | cut -f1)
        echo "  [OK] pista_caudina ($size)"
    else
        echo "  [!!] pista_caudina (kn5 mancante!)"
    fi
else
    echo "  [--] pista_caudina (non installata)"
fi
echo ""
...
echo "Seleziona 'Pista Caudina' dal menu piste."
